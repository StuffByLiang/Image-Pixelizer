...
TEST_CASE("stats::basic rectArea","[weight=1][part=stats]"){
    ...
    stats s(data);
    long result = s.rectArea(2);

    REQUIRE(result == 16);
    
}

TEST_CASE("stats::basic getSum","[weight=1][part=stats]"){

    PNG data; data.resize(2,2);
    *data.getPixel(0, 0) = RGBAPixel(1, 1, 1);
    *data.getPixel(1, 0) = RGBAPixel(2, 2, 2);
    *data.getPixel(0, 1) = RGBAPixel(3, 3, 3);
    *data.getPixel(1, 1) = RGBAPixel(4, 4, 4);
    stats s(data);

    REQUIRE(1 == s.getSum('r', {0, 0}, 0));
    REQUIRE(2 == s.getSum('r', {1, 0}, 0));
    REQUIRE(3 == s.getSum('r', {0, 1}, 0));
    REQUIRE(4 == s.getSum('r', {1, 1}, 0));
